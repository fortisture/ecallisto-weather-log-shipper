...
from station import api, receiver, status  # noqa: E402
...
def test_csv_robustness(tmp):
    ...
    # Header only, no rows.
    write(os.path.join(store, "2026", "01", "02", "header_20260102.csv"),
          "timestamp,temp_c\n")

    # No timestamp column at all -- must be skipped, not crash.
    write(os.path.join(store, "2026", "01", "03", "nots_20260103.csv"),
          "foo,bar\n1,2\n")

    # Ragged rows: too few and too many fields.
    write(os.path.join(store, "2026", "01", "04", "ragged_20260104.csv"),
    ...
    write(os.path.join(store, "2026", "01", "05", "junk_20260105.csv"),
          "timestamp,temp_c,humidity_pct\n"
    ...
    write(os.path.join(store, "2026", "01", "06", "badts_20260106.csv"),
          "timestamp,temp_c\n"
          "yesterday,5\n"
    ...
    write_bytes(os.path.join(store, "2026", "01", "07", "bom_20260107.csv"),
                b"\xef\xbb\xbftimestamp,temp_c\r\n2026-01-07 00:00:00,9.5\r\n")

    # Latin-1 bytes that are not valid UTF-8.
    write_bytes(os.path.join(store, "2026", "01", "08", "latin_20260108.csv"),
    ...
    write(os.path.join(store, "2026", "01", "09", "wild_20260109.csv"),
    ...
    write(os.path.join(store, "2026", "01", "10", "order_20260110.csv"),
          "timestamp,temp_c\n"
    ...
    check("skips files without a timestamp column",
          all("foo" not in r for r in readings))

    stamps = [r["timestamp"] for r in readings]
    check("output is sorted chronologically", stamps == sorted(stamps))

    check("BOM did not corrupt the first column",
          any(r["timestamp"].startswith("2026-01-07") for r in readings),
          "a BOM on the header makes the first column name '\\ufefftimestamp'")

    check("non-UTF-8 bytes did not stop the file",
          any(r["timestamp"].startswith("2026-01-08") for r in readings))

    check("unparseable timestamps dropped, good rows kept",
          any(r["timestamp"].startswith("2026-01-06T03") for r in readings))

    check("non-numeric values become null, not zero",
          all(r["temp_c"] != 0.0 or r["temp_c"] is None
              for r in readings if r["timestamp"].startswith("2026-01-05T00")))
...
def test_filename_safety():
    ...
    check("rejects a null byte in the name",
          receiver.safe_filename("normal.csv\x00.txt") is None or
          "\x00" not in (receiver.safe_filename("normal.csv\x00.txt") or ""),
          "a NUL can truncate the path in some filesystem layers")

    check("blob names must carry a known extension",
          receiver.safe_blob_name("evil.sh") is None and
          receiver.safe_blob_name("x.fit.gz") == "x.fit.gz")

    check("dates are read out of filenames",
          receiver.date_subdir("Croatia-Visnjan_20250909_075911_03.fit.gz")
          == ("2025", "09", "09"))

    check("an impossible date is not accepted",
          receiver.date_subdir("x_20259999_000000_00.fit.gz") is None)

    check("undated files are filed as undated, not guessed",
...
def test_processing():
    ...
    for i in range(200):
        ...
        readings.append({
            "timestamp": (base + timedelta(minutes=offset)).strftime("%Y-%m-%dT%H:%M:%SZ"),
            "temp_c": float(i),
            "humidity_pct": 50.0,
            "pressure_hpa": 1013.0,
            "dew_point_c": 5.0,
    ...
    check("an outage produces exactly one gap marker", len(gaps) == 1,
          f"got {len(gaps)}")
    check("the gap marker carries no values",
          all(gaps[0].get(k) is None for k in ("temp_c", "humidity_pct")) if gaps else False)

    fields = ("temp_c", "humidity_pct", "pressure_hpa", "dew_point_c")
    reduced = api.downsample(marked, 50, fields)
    check("downsampling respects the point budget", len(reduced) <= 55,
          f"asked for 50, got {len(reduced)}")
    check("downsampling preserves gap markers",
          any(r.get("gap") for r in reduced))

    values = [r["temp_c"] for r in reduced if r.get("temp_c") is not None]
    check("averaging keeps the data's range",
    ...
    check("one reading survives downsampling",
    ...
    check("slice_hours is anchored on the newest reading",
          sliced and sliced[-1] == readings[-1])
    check("slice_hours(0) means everything",
...
def test_sun():
    ...
    def ev(d):
        return status._solar_events(date.fromisoformat(d),
                                    status.STATION_LAT, status.STATION_LON)

    summer = ev("2026-06-21")
    winter = ev("2026-12-21")
    equinox = ev("2026-03-20")

    check("solar noon is near 11:00 UTC for 13.7E",
          "10:5" in summer["solar_noon"] or "11:0" in summer["solar_noon"]
          or "11:1" in summer["solar_noon"],
          f'got {summer["solar_noon"]}')

    check("longest day is about 15.6 h at 45N",
          15.4 < summer["day_length_hours"] < 15.9,
          f'got {summer["day_length_hours"]}')

    check("shortest day is about 8.7 h at 45N",
          8.5 < winter["day_length_hours"] < 9.0,
          f'got {winter["day_length_hours"]}')

    check("equinox day is close to 12 h",
          11.8 < equinox["day_length_hours"] < 12.4,
          f'got {equinox["day_length_hours"]}')

    check("sunrise precedes solar noon precedes sunset",
          summer["sunrise"] < summer["solar_noon"] < summer["sunset"])

    check("civil dawn precedes sunrise",
          summer["civil_dawn"] < summer["sunrise"])
    ...
    check("polar day returns None rather than inventing a sunrise",
...
def test_status():
    ...
    check("an hour late reads as late", stale["state"] == "late",
          f'got {stale["state"]}')
    ...
    check("a clock ahead of the server does not read as down",
          future["state"] == "up", f'got {future["state"]}, age {future["age_seconds"]}')
...
def test_generation(tmp):
    ...
    for i in range(500):
        ...
        rows.append(f"{t:%Y-%m-%d %H:%M:%S},{20 + i % 10}.0,{50 + i % 20},{1010 + i % 5}.0")
    write(os.path.join(weather, "2026", "08", "01", "w_20260801.csv"), "\n".join(rows) + "\n")

    prows = ["timestamp,heater_v,heater_ma,lna_v,lna_ma,callisto_v,callisto_ma,bme_v,bme_ma"]
    for i in range(300):
        t = base + timedelta(minutes=15 * i)
        prows.append(f"{t:%Y-%m-%d %H:%M:%S},12.1,{100 + i % 50},12.0,90,12.0,420,3.3,3.1")
    write(os.path.join(power, "2026", "08", "01", "power_20260801.csv"), "\n".join(prows) + "\n")
    ...
    for slug, _ in api.WEATHER_RANGES:
        ...
        if len(d["readings"]) > api.MAX_POINTS_PER_RANGE + 10:
            check(f"weather range {slug} respects the budget", False,
    ...
    with open(os.path.join(out, "weather", "latest.json"), encoding="utf-8") as f:
        latest = json.load(f)
    check("latest is a real reading, not a gap marker",
          latest.get("temp_c") is not None)

    with open(os.path.join(out, "power", "latest.json"), encoding="utf-8") as f:
        plat = json.load(f)
    check("power totals are derived, not stored",
    ...
    check("watts equal volts x amps exactly", abs(watts - expected) < 0.01,
          f"{watts} vs {expected}")
...
def test_idempotence(tmp):
    ...
    import filecmp
    same = filecmp.cmp(os.path.join(out1, "weather", "history-24h.json"),
                       os.path.join(out2, "weather", "history-24h.json"),
                       shallow=False)
    check("the same input always produces the same output", same,
